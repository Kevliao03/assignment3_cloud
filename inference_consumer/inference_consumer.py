...
def update_couchdb(image_id, predicted_class):
    ...
    if response.status_code == 200:
        ...
        update_response = requests.put(f"{doc_url}?rev={doc['_rev']}", headers=headers, data=json.dumps(doc))

        if update_response.status_code == 201:
            print(f"Document {image_id} successfully updated.")
        else:
            print(f"Failed to update document {image_id}. Error: {update_response.text}")
    else:
